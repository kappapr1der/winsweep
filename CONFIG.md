...
Секция `automation` делает фоновую уборку спокойнее, чем ручную:

- `standardFreeGB`: ниже этого остатка включается обычный режим с кэшем старше 3 дней.
- `emergencyFreeGB`: ниже этого остатка включается экстренный режим без ожидания по возрасту.
- `lightCacheOlderThanDays`: между основным порогом и `standardFreeGB` чистится только кэш старше этого возраста.
- `standardCacheOlderThanDays`: возраст кэша для обычного режима.
- `activeSpotifyCacheOlderThanDays`: пока Spotify открыт, остаётся только очистка старого музыкального кэша; активные browser/code-кэши пропускаются.
- `notificationMinimumMB`: не показывать системное уведомление, если освобождено меньше указанного объёма.

Кэши браузеров, Discord, Telegram, Razer и игровых лаунчеров пропускаются,
если соответствующее приложение открыто. Это записывается в журнал как нормальный
пропуск и не считается ошибкой.
