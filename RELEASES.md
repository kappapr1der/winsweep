...
```powershell
.\build-release.ps1 -Version 1.1.3 -Portable
```

Готовый файл появится в `dist\WinSweep-Portable-v1.1.3.zip`. После первого
...
```powershell
.\build-release.ps1 -Version 1.1.3
```
...
```powershell
.\publish-release.ps1 -Version 1.1.3 -Portable
```

Сценарий собирает ZIP, создаёт или использует тег `v1.1.3` и прикрепляет архив
...
```powershell
.\publish-release.ps1 -Version 1.1.3 -Portable -DryRun
.\publish-release.ps1 -Version 1.1.3 -Portable -Prerelease
.\publish-release.ps1 -Version 1.1.3 -Portable -UpdateExisting -ReplaceAsset
