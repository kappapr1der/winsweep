...
Скачай `WinSweep-v1.0.0.zip`, распакуй его и запусти единственный файл:

```text
WinSweep.exe
```

На первом запуске WinSweep сам развернёт внутренний движок в профиле пользователя
и откроет единое окно. Консоль, батники и ручной запуск PowerShell не нужны.
...
В режиме EXE WinSweep хранит рабочий движок и конфигурацию в
`%LOCALAPPDATA%\WinSweep\Engine`. Настройки можно менять из вкладки
`Кэши и правила`; пользовательский конфиг сохраняется между обновлениями EXE.
